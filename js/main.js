...
(function () {
  ...
  */
  ...
  /* Di CANVAS dipakai versi kecil dari images/thumbs (dibuat oleh
     tools/make-thumbs.js) — jauh lebih ringan. Versi ASLI hanya dipakai
     saat karya diklik. Urutan percobaan: webp -> jpg kecil -> asli. */
  function tileSources(origUrl) {
    const base = origUrl.replace(/^images\//, "").replace(/\.(jpe?g|png)$/i, "");
    return [
      "images/thumbs/" + base + ".webp",
      "images/thumbs/" + base + ".jpg",
      realURL(origUrl),
    ];
  }

  /* Pasang daftar alamat cadangan: kalau satu gagal, coba berikutnya. */
  function attachFallback(img, list) {
    let i = 0;
  ...
  function makeTileEl(p) {
    ...
    if (p.img) {
      ...
      const srcs = tileSources(p.img);
      img.dataset.src = attachFallback(img, srcs);
    ...
    if (p.video && p.vid) {
      ...
      vid.preload = "none";
      /* Poster = frame pertama video (dibuat tools/compress-videos.js).
         Tile langsung menampilkan gambar walau videonya belum dimuat. */
      vid.poster = "images/thumbs/" +
                   p.vid.replace(/^images\//, "").replace(/\.mp4$/i, "") + ".jpg";
      vid.src = p.vid;
  ...
  function syncMedia() {
    ...
    /* 2. Video — semua yang terlihat tetap diputar otomatis (web & HP),
       TAPI unduhannya diantrikan, tidak serentak.

       Kenapa: satu klip bisa 10 MB lebih. Kalau 4 video diunduh
       bersamaan, jaringan HP langsung penuh dan SELURUH halaman
       (termasuk gambar) ikut tertahan — inilah yang bikin beranda
       terasa "loading terus". Dengan antrian, gambar tampil dulu,
       lalu video menyusul satu per satu. */
    ...
    for (let i = 0; i < videoEls.length; i++) {
      ...
      // sudah siap / sudah jalan -> biarkan
      if (!v.paused) continue;
      if (v.readyState >= 3) { v.play().catch(() => {}); continue; }
      // masih mengunduh?
      if (v.__loading) { loading++; continue; }
      waiting.push(v);
    }

    // berapa video boleh mengunduh bersamaan
    const SLOTS = small ? 1 : 2;
    for (let i = 0; i < waiting.length && loading < SLOTS; i++) {
      const v = waiting[i];
      v.__loading = true;
      loading++;
      const clear = () => { v.__loading = false; };
      v.addEventListener("canplay", clear, { once: true });
      v.addEventListener("error", clear, { once: true });
      // kalau macet, jangan menahan antrian selamanya
  ...
  canvas.addEventListener("click", (e) => {
    ...
    if (p.video && p.vid) {
      ...
      if (img) {
        ...
        // tampilan besar memakai file ASLI (kualitas penuh)
        big.src = realURL(p.img);
        big.alt = p.title;
