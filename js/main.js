...
(function () {
  ...
  */
  ...
  /* Bentuk tile di canvas MENGIKUTI rasio asli file yang diupload
     (diukur saat loading). Angka di bawah hanya dipakai sementara
     kalau file belum sempat terukur. */
  const RATIOS_IMG = [1, 0.6, 1, 0.75, 1.1, 0.8, 1.45, 0.7, 0.9, 1.25, 0.65, 1.2];
  const RATIOS_VID = [1.6, 0.62, 1.8, 0.72, 1.45, 0.56, 1.7, 0.66];

  /* Pengaman rasio: file yang ekstrem panjang/lebar tetap dibatasi
     supaya satu tile tidak memenuhi seluruh kolom. 0.18 ≈ 1:5.5 */
  const MIN_RATIO = 0.18;
  const MAX_RATIO = 4.5;

  /* Jarak acak tambahan antar tile (px), di atas GAP normal.
     Bikin susunan terasa acak tapi tidak pernah menganga lebar. */
  ...
  */
  ...
  function makeTileEl(p) {
    ...
    // dipakai bersama content-visibility:auto — browser tahu ukuran
    // tile yang sedang di luar layar tanpa perlu merendernya
    ...
    if (p.img) {
      ...
      /* Gambar BARU diunduh saat tile-nya mendekati layar (lihat
         ...
         di sini karena salah membaca posisi tile di canvas ber-transform,
         jadi penundaannya diatur sendiri lewat data-src. */
  ...
  function syncMedia() {
    ...
    // 1. mulai unduh gambar yang sudah mendekat (margin agak lebar
    //    supaya sudah siap sebelum benar-benar terlihat)
    ...
    /* 2. Video. File video jauh lebih berat daripada gambar (bisa
       belasan MB), jadi aturannya lebih ketat:
         - hanya yang benar-benar terlihat
         - harus DIAM di layar beberapa saat dulu, supaya video yang
           cuma terlewat saat menggeser tidak ikut diunduh
         - jumlah yang berputar bersamaan dibatasi (HP lebih ketat) */
    const MV = 60;
    const DWELL = small ? 700 : 400;   // ms harus terlihat dulu
    const MAX_PLAYING = small ? 2 : 4;
    const now = performance.now();
    const busy = dragging || Math.abs(velX) + Math.abs(velY) > 2;
    let playing = 0;
    ...
    for (let i = 0; i < videoEls.length; i++) {
      ...
      if (!v.__seenAt) v.__seenAt = now;

      const settled = now - v.__seenAt >= DWELL && !busy;
      if (settled && playing < MAX_PLAYING) {
        playing++;
        if (v.paused) v.play().catch(() => {});
      } else if (!v.paused && playing >= MAX_PLAYING) {
        v.pause();
      }
    }
  }

  /* Gambar di dalam modal (skill & client) baru diunduh saat
     modal-nya dibuka — tidak ikut membebani saat website dibuka. */
  ...
  function build() {
    ...
    // wrap period of each column (ikut menghitung jarak acak tiap tile)
  ...
  /* ── Mengukur ukuran asli tiap file ─────── */
  ...
  /* ── Preloader + intro ────────────────────
     Bar progres sekarang mengikuti proses NYATA: tiap file diukur
     rasionya lebih dulu supaya canvas langsung tersusun benar dan
     gambar sudah ada di cache (tidak muncul satu per satu). */
  function runPreloader() {
    const MIN_MS = 800;           // biar animasi loader sempat terlihat
    const DEADLINE_MS = 6000;     // jangan menunggu selamanya

    /* Hanya file yang ukurannya BELUM diketahui yang perlu diukur.
       Kalau js/media-sizes.js sudah lengkap, daftar ini kosong dan
       website langsung siap tanpa mengunduh apa pun dulu. */
    const todo = [];
    const seen = {};
    projects.forEach((p) => {
      const u = mediaURL(p);
    ...
    const iv = setInterval(() => {
      ...
      // tanpa file yang perlu diukur, bar mengikuti waktu animasi saja
      const real = total === 0 ? elapsed / MIN_MS : measured / total;
      // naik halus, maksimal 4% per frame — tidak melompat-lompat
  ...
  // selesai diukur — jadi canvas langsung tersusun dengan bentuk benar
  runPreloader();
